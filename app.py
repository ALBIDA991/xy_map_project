...
def dedupe_allowed():
    ...
    conn = sqlite3.connect('coords.db')
    c = conn.cursor()
    c.execute("SELECT x, y FROM allowed_coords")
    coords = c.fetchall()
    cell_size = 111
    seen_cells = set()
    to_delete = []
    ...
    for x, y in to_delete:
        c.execute("DELETE FROM allowed_coords WHERE x=? AND y=?", (x, y))
    conn.commit()
    conn.close()
    return f"{len(to_delete)} 件の重複座標を削除しました（9×9マス単位）"

def seed_allowed_coords():
    conn = sqlite3.connect('coords.db')
    c = conn.cursor()
    c.execute("""
        CREATE TABLE IF NOT EXISTS allowed_coords (
            x INTEGER,
            y INTEGER,
            level INTEGER,
            PRIMARY KEY (x, y)
